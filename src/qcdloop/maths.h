...
namespace ql
{
  ...
  inline qdouble  Abs(qdouble const& x)   { return std::fabs(x); }
  inline qdouble  Abs(qcomplex const& x)  { return std::fabs(x);}

  inline qdouble  Imag(qdouble const& x)  { UNUSED(x); return 0; }
  inline qdouble  Imag(qcomplex const& x) { return x.imag(); }

  inline qdouble  Real(qdouble const& x) { return x; }
  inline qdouble  Real(qcomplex const& x) { return x.real(); }
